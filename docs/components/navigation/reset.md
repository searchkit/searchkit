...
- `options` *(object)*: An object containing `{query:true|false, filter:true|false}` which controls which parts of the search are reset and influence the disabled state.
