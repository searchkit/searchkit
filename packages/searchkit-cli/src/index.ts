import { addMappingES7, createIndices, dropIndices, getMapping, getSearchkitConfig, getSKQuickStartText, indexDocs } from './lib'
...
export const withConfig = async (config: CliConfig) => {
  ...
  await inquirer.prompt({
    ...
    if (chosenAnswer) {
      ...
      const c = getSKQuickStartText({ ...skConfig, host: config.host, index: config.index, mapping })
