...
var RangeAccessor = (function (_super) {
    ...
    RangeAccessor.prototype.buildSharedQuery = function (query) {
        ...
        if (this.state.hasValue()) {
            ...
            var rangeFilter = query_1.RangeQuery(this.options.field, val.min, val.max);
            var selectedFilter = {
    ...
    RangeAccessor.prototype.buildOwnQuery = function (query) {
        query = query.setAggs(query_1.FilterBucket(this.key, query.getFiltersWithoutKeys(this.key), query_1.HistogramBucket(this.key, this.options.field, {
