...
var NestedFacetAccessor = (function (_super) {
    ...
    NestedFacetAccessor.prototype.buildSharedQuery = function (query) {
        ...
        if (filterTerms.length > 0) {
            var leafFilter = _.get(levelFilters, [levelFilters.length - 1, 0], "");
            var parentOfleaf = _.get(levelFilters, [levelFilters.length - 2, 0], this.options.title || this.key);
            var selectedFilter = {
                id: this.key,
    ...
    NestedFacetAccessor.prototype.buildOwnQuery = function (query) {
        ...
        return query.setAggs(query_1.FilterBucket(this.key, query.getFilters(this.uuid), query_1.NestedBucket.apply(void 0, ["children", this.options.field].concat(lvlAggs))));
    };
