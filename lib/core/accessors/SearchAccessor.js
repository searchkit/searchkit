...
var SearchAccessor = (function (_super) {
    ...
    SearchAccessor.prototype.buildSharedQuery = function (query) {
        ...
        if (queryStr) {
            var simpleQuery = QueryBuilders_1.QueryString(queryStr, {
                "query": queryStr,
                "type": "best_fields",
            ...
            var prefixQueries = this.options.prefixSearch ? _.map(this.options.queryFields, QueryBuilders_1.MatchPhrasePrefix.bind(null, queryStr)) : [];
            var queries = [].concat(QueryBuilders_1.BoolShould(prefixQueries)).concat(simpleQuery);
