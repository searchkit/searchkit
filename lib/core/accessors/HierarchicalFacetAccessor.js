...
var HierarchicalState = (function (_super) {
    ...
    HierarchicalState.prototype.removeChilds = function (level) {
        var _this = this;
        debugger;
        var c = _.map(_.range(level + 1, this.getLeafLevel()), function (level) {
            _this.clear(level);
        });
        ...
        _.each(this.options.fields, function (field, i) {
            ...
            var filterTerms = _.map(filters, function (filter, idx) {
                return QueryBuilders_1.Term(field, filter, {
                    $name: parentFilter[0] || _this.options.title || field,
                    $value: filter,
                    $id: _this.options.id,
                    $remove: function () {
