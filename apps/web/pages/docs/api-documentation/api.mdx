...
To see the full Elasticsearch query that is executed to Elasticsearch, you can run the API in debug mode (see below).
...
The API can be run in debug mode to help with debugging the Elasticsearch query. To run the API in debug mode, set the `debug` flag to `true` in the `handleRequest` function.
...
When the API is run in debug mode, the Elasticsearch query is logged to the console.